...
export default function ClientLogoSlider({ clients }: ClientLogoSliderProps) {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % (clients.length - visibleLogos + 1))
  }

  const prevSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex === 0 ? clients.length - visibleLogos : prevIndex - 1))
  }
    ...
    <div className="relative w-full overflow-hidden px-4 py-8">
      ...
      <div ref={containerRef} className="overflow-hidden">
        <motion.div
          ...
          animate={{ x: (-currentIndex * (containerRef.current?.clientWidth || 0)) / visibleLogos }}
      ...
      <div className="flex justify-center mt-4">
        {Array.from({ length: Math.min(clients.length - visibleLogos + 1, 9) }).map((_, index) => (
          <button
            key={index}
